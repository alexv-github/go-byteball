...
type(
	DefinitionT = types.DefinitionT
)

//function hasReferences(arrDefinition){
func HasReferences(arrDefinition DefinitionT) bool {
	var evaluate func (arr []interface{}) bool

	evaluate = func (arr []interface{}) bool {
		op := arr[0]
		args := arr[1]
...
//			for (var i=0; i<args.length; i++)
		...
		case "r of set":
			set := (args.(map[string] interface{}))["set"].([]interface{})
//			for (var i=0; i<args.set.length; i++)
			for _, set_i := range set {
//				if (evaluate(args.set[i]))
				if evaluate(set_i.([]interface{})) {
					return true
				}
			}
			return false
		...
		case "weighted and":
			set := (args.(map[string] interface{}))["set"].([]map[string] []interface{})
//			for (var i=0; i<args.set.length; i++)
			for _, set_i := range set {
//				if (evaluate(args.set[i].value))
				if (evaluate(set_i["value"])) {
					return true
				}
			}
			return false
		...
		case "not":
			return evaluate(args.([]interface{}))
...
//			throw Error("unknown op: "+op);
			_core.Throw("unknown op: %s", op)
		}

		return true
	}
	
	return evaluate(arrDefinition.([]interface{}))
}
